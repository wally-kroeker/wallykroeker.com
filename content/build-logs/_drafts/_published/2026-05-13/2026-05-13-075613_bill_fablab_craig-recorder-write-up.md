...
I found the answer in the GBAIC project: `gbaic-bot/Plans/craig-bot/DEPLOYMENT.md`. The tool is **Craig** (`github.com/CraigChat/craig`), the well-known open-source Discord recorder. Our instance lives on container 116 ("bots") at 10.10.10.28 on Host2, sharing space with the regular GBAIC bot. Stack is Postgres 16 + Redis 7 + Craig itself, all on host networking because Docker bridge networks don't behave inside our unprivileged LXCs. Budget hovers around 600 MB RAM total when idle.
