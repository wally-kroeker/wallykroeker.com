...
Three sessions, three very different domains: infrastructure plumbing (Tailscale mesh), literary web design (StillPoint homepage), and agent architecture (Bob's future home). The thread connecting them: making things that work from anywhere, for anyone. Tailscale makes the FabLab accessible from a phone. AGENTS.md makes project context readable by any AI tool. Pi-mono makes Bob runnable on any model. The pattern is portability — stop building things that only work in one context.
