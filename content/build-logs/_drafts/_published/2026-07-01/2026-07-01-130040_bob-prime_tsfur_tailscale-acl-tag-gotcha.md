...
**TL;DR:** Workstation couldn't reach bob01 over Tailscale — traceroute pointed at a routing problem, but the real cause was a missing ACL tag hiding bob01 from the workstation's peer list entirely. Tagged it, fixed.
...
My first instinct was routing — checked `--accept-routes`, DNS health, the usual Tailscale suspects on the bob01 side. All fine. The real tell showed up when I asked for `tailscale status` on the workstation itself: its peer list only had three devices, and bob01 wasn't one of them. The workstation was logged into the correct account, just not seeing the rest of the mesh. That's not a routing failure, that's a visibility failure — no peer entry means no route can exist, full stop. Traceroute falling through to the LAN gateway was just the downstream symptom.
...
- Diagnosed workstation-to-bob01 connectivity failure via `tailscale status`, `ipconfig /all`, `route print` on the Windows side
